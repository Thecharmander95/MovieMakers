...
class ScenesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create scene" do
    assert_difference('Scene.count') do
      post scenes_url, params: { scene: { content: @scene.content, movie_id: @scene.movie_id, name: @scene.name } }
    end
  ...
  test "should update scene" do
    patch scene_url(@scene), params: { scene: { content: @scene.content, movie_id: @scene.movie_id, name: @scene.name } }
